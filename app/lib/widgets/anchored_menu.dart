...
class MenuOption<T> {
  ...
  const MenuOption({required this.id, required this.label, required this.icon, this.tag = ''});
}
...
Future<T?> showAnchoredCard<T>({
  required BuildContext context,
  required String title,
  required Widget Function(BuildContext context, void Function(T? result) close) body,
}) {
  final box = context.findRenderObject() as RenderBox?;
  final overlay = Overlay.of(context).context.findRenderObject() as RenderBox?;
  Rect anchor = Rect.zero;
  ...
  return Navigator.of(context).push<T>(
    _AnchoredMenuRoute<T>(
      anchor: anchor,
      builder: (context, close) => _MenuCard(title: title, children: [body(context, close)]),
    ),
  );
}
    ...
    return Material(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: const [BoxShadow(color: Color(0x99000000), blurRadius: 40, offset: Offset(0, 12))],
        ),
        child: Column(
...
class _MenuOptionRow<T> extends StatelessWidget {
  ...
  const _MenuOptionRow({required this.option, required this.selected, required this.onTap});
  ...
  Widget build(BuildContext context) {
    return InkWell(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            SizedBox(width: 16, child: Icon(option.icon, size: 13, color: AppColors.textFaint)),
            const SizedBox(width: 10),
            Expanded(child: Text(option.label, style: AppTheme.sans(size: 13))),
            if (option.tag.isNotEmpty)
              Padding(
                padding: const EdgeInsets.only(left: 8),
                child: Text(option.tag, style: AppTheme.mono(size: 9.5, color: AppColors.textMuted)),
              ),
            if (selected)
              const Padding(
  ...
  Widget buildPage(BuildContext context, Animation<double> animation, Animation<double> secondary) {
    ...
    return FadeTransition(
      ...
      child: CustomSingleChildLayout(
        delegate: _AnchorLayout(anchor, MediaQuery.of(context).viewInsets.bottom),
...
class _AnchorLayout extends SingleChildLayoutDelegate {
  ...
  BoxConstraints getConstraintsForChild(BoxConstraints constraints) =>
      BoxConstraints.loose(Size(constraints.maxWidth - 16, constraints.maxHeight - 16));
  ...
  Offset getPositionForChild(Size size, Size childSize) {
    ...
    left = left.clamp(8.0, (size.width - childSize.width - 8).clamp(8.0, double.infinity));
